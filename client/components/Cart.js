...
import { fetchUser, fetchUserCart, deleteItemCartThunk, updateQuantityThunk } from '../redux/singleUser';
    ...
    return (
      ...
      <div>This is {user.username}'s Cart!</div>
        <div>{cartItems.map((item) => (
            <div key={item.id}>
            <button onClick={() => this.clickDelete({ userId: user.id, productId: item.id })} id="x-button">
                X
              </button>
            ...
            <p>QUANTITY: {item.quantity}</p>
             <div>
            <button onClick={() => this.incrementItem({userId: user.id, productId: item.id, quantity: item.quantity + 1 })} type="button">
              Click to increment by 1
            </button>
            <button onClick={() => this.decrementItem({userId: user.id, productId: item.id, quantity: item.quantity -1  })} type="button">
